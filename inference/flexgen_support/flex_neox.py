...
from neox_config import NeoXConfig, get_neox_config, download_neox_weights
from pytorch_backend import (TorchDevice, TorchDisk, TorchLink,
    TorchMixedDevice, DeviceType, general_copy, fix_recursive_import)
from flexgen_timer import timers
...
def init_weight_list(weight_specs, policy, env):
    logging.debug("<init_weight_list> enter.")
...
class InputEmbed:
    def __init__(self, config, env, policy):
        logging.debug(f"<InputEmbed.__init__> enter.")
        self.config = config
        self.env = env
        self.policy = policy
    ...
    def init_weight(self, weight_home, path):
        logging.debug(f"<InputEmbed.init_weight> enter.")
        v, h, dtype = (self.config.vocab_size, self.config.input_dim, self.config.dtype)
        path = os.path.join(path, "")
        weight_specs = [
            # embed_in
            ((v, h), dtype, path + "gpt_neox.embed_in.weight"),
        ]
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        logging.debug(f"<InputEmbed.load_weight> enter.")
        embed_in, = weight_home.val
        if k == 0:
            dst = self.weight_load_dst
            weight_read_buf.store((embed_in.smart_copy(dst),))
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        logging.debug(f"<InputEmbed.forward> enter.")
        ...
        if k == self.policy.num_gpu_batches - 1:
            ...
            (embed_in, donate[2]), = weight_read_buf.pop()
        else:
            (embed_in, _) = weight_read_buf.val

        h = self.compute.neox_input_embed(h, mask,
            embed_in, self.config.pad_token_id, donate)
        hidden.val = h


class OutputEmbed:
    def __init__(self, config, env, policy):
        logging.debug(f"<OutputEmbed.__init__> enter.")
        self.config = config
        self.env = env
        self.policy = policy
        self.compute = self.env.gpu
    ...
    def init_weight(self, weight_home, path):
        logging.debug(f"<OutputEmbed.init_weight> enter.")
        v, h, dtype = (self.config.vocab_size, self.config.input_dim,
            self.config.dtype)
        path = os.path.join(path, "")
        weight_specs = [
            # w_final_ln
            ((h,), dtype, path + "gpt_neox.final_layer_norm.weight"),
            # b_final_ln
            ((h,), dtype, path + "gpt_neox.final_layer_norm.bias"),
            # embed_out
            ((v, h), dtype, path + "embed_out.weight"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        logging.debug(f"<OutputEmbed.load_weight> enter.")
        w_final_ln, b_final_ln, embed_out = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((w_final_ln.smart_copy(dst2), b_final_ln.smart_copy(dst2),
                embed_out.smart_copy(dst1)))
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        logging.debug(f"<OutputEmbed.forward> enter.")
        donate = [False] * 4
        h, donate[0] = hidden.val, True
        ...
        h = self.compute.neox_output_embed(h, w_ln, b_ln, w_token, donate,
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id):
        logging.debug(f"<SelfAttention.__init__> enter.")
        self.config = config
        self.env = env
    ...
    def init_weight(self, weight_home, path):
        logging.debug(f"<SelfAttention.init_weight> enter (layer {self.layer_id}).")
        h, s, dtype = (self.config.input_dim, self.config.max_position_embeddings, self.config.dtype)
        path = os.path.join(os.path.join(path, f"gpt_neox.layers.{self.layer_id}."))
        weight_specs = [
            # w_in_ln
            ((h,), dtype, path + "input_layernorm.weight"),
            # b_in_ln
            ((h,), dtype, path + "input_layernorm.bias"),
            # a_bias
            ((1, 1, s, s), bool, path + "attention.bias"),
            # inv_freq
            ((12,), dtype, path + "attention.rotary_emb.inv_freq"),
            # w_qkv
            ((3*h, h), dtype, path + "attention.query_key_value.weight"),
            # b_qkv
            ((3*h,), dtype, path + "attention.query_key_value.bias"),
            # w_o
            ((h, h), dtype, path + "attention.dense.weight"),
            # b_o
            ((h,), dtype, path + "attention.dense.bias"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
        weight_home.store(weights)

    def load_weight(self, weight_home, weight_read_buf, k):
        logging.debug(f"<SelfAttention.load_weight> enter.")
        w_in_ln, b_in_ln, a_bias, inv_freq, w_qkv, b_qkv, w_o, b_o = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
                w_in_ln.smart_copy(dst2), b_in_ln.smart_copy(dst2),
                a_bias.smart_copy(dst1), inv_freq.smart_copy(dst2),
                w_qkv.smart_copy(dst1), b_qkv.smart_copy(dst2),
                w_o.smart_copy(dst1), b_o.smart_copy(dst2)))

    def init_cache_one_gpu_batch(self, cache_home):
        logging.debug(f"<SelfAttention.init_cache_one_gpu_batch> enter.")
    ...
    def load_cache(self, cache_home, cache_read_buf, i):
        logging.debug(f"<SelfAttention.load_cache> enter.")
    ...
    def store_cache(self, cache_home, cache_write_buf, i):
        logging.debug(f"<SelfAttention.store_cache> enter.")
        # shape: (s, b * n_head, head_dim)
        k_home, v_home = cache_home.val
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        logging.debug(f"<SelfAttention.forward> enter.")
        n_head = self.config.n_head
        
        rotary_ndims = int(self.config.hidden_size / self.config.n_head * self.config.rotary_pct)
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((w_in_ln, donate[2]), (b_in_ln, donate[3]), (a_bias, donate[4]), (inv_freq, donate[5]),
             (w_qkv, donate[6]), (b_qkv, donate[7]), (w_o, donate[8]), (b_o, donate[9])
             ) = weight_read_buf.pop()
        else:
            ((w_in_ln, _), (b_in_ln, _), (a_bias, _), (inv_freq, _),
             (w_qkv, _), (b_qkv, _), (w_o, _), (b_o, _)
             ) = weight_read_buf.val

        if i == 0:  # prefill
            mask, donate[1] = attention_mask.val.smart_copy(self.compute)
            h, new_k_cache, new_v_cache = self.compute.mha(h, mask, 
                w_in_ln, b_in_ln, a_bias, inv_freq, w_qkv, b_qkv, w_o, b_o, n_head, rotary_ndims, self.config.max_position_embeddings,
            ...
            h, new_k_cache, new_v_cache = self.compute.mha_gen(h, mask, 
                w_in_ln, b_in_ln, a_bias, inv_freq, w_qkv, b_qkv, w_o, b_o, n_head, rotary_ndims, self.config.max_position_embeddings,
...
class MLP:
    def __init__(self, config, env, policy, layer_id):
        logging.debug(f"<MLP.__init__> enter.")
        self.config = config
        self.env = env
        self.layer_id = layer_id
        self.policy = policy
    ...
    def init_weight(self, weight_home, path):
        logging.debug(f"<MLP.init_weight> enter (layer {self.layer_id}).")
        h, dtype = (self.config.input_dim, self.config.dtype)
        path = os.path.join(os.path.join(path, f"gpt_neox.layers.{self.layer_id}."))
        weight_specs = [
            # w_i
            ((4 * h, h), dtype, path + "mlp.dense_h_to_4h.weight"),
            # b_i
            ((4 * h,), dtype, path + "mlp.dense_h_to_4h.bias"),
            # w_o
            ((h, 4 * h), dtype, path + "mlp.dense_4h_to_h.weight"),
            # b_o
            ((h,), dtype, path + "mlp.dense_4h_to_h.bias"),
            # w_post_ln
            ((h,), dtype, path + "post_attention_layernorm.weight"),
            # b_post_ln
            ((h,), dtype, path + "post_attention_layernorm.bias"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
        weight_home.store(weights)

    def load_weight(self, weight_home, weight_read_buf, k):
        logging.debug(f"<MLP.load_weight> enter.")
        w_i, b_i, w_o, b_o, w_post_ln, b_post_ln = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
                w_i.smart_copy(dst1), b_i.smart_copy(dst2),
                w_o.smart_copy(dst1), b_o.smart_copy(dst2),
                w_post_ln.smart_copy(dst2), b_post_ln.smart_copy(dst2)))
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        logging.debug(f"<MLP.forward> enter.")
        donate = [False] * 7
        h, donate[0] = hidden.val, True
...
class TransformerLayer:
    def __init__(self, config, env, policy, i):
        logging.debug(f"<TransformerLayer.__init__> enter.")
    ...
    def init_weight(self, weight_home, path):
        logging.debug(f"<TransformerLayer.init_weight> enter.")
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        logging.debug(f"<TransformerLayer.load_weight> enter.")
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        logging.debug(f"<TransformerLayer.forward> enter.")
        if k == self.policy.num_gpu_batches - 1:
...
class NeoXLM:
    def __init__(self,
                 config: Union[str, NeoXConfig],
                 env: ExecutionEnv,
                 path: str,
                 policy: Policy):
        logging.debug(f"<NeoXLM.__init__> enter.")
        if isinstance(config, str):
            config = get_neox_config(config)
        self.config = config
        self.env = env
        self.path = path
        self.policy = policy
    ...
    def init_weight(self, j):
        logging.debug(f"<NeoXLM.init_weight> enter. self.path: {self.path}")
        #expanded_path = os.path.abspath(os.path.expanduser(os.path.join(self.path, f"{self.config.name}-np")))
        expanded_path = os.path.join(self.path, f"{self.config.name}-np")
        check_path = os.path.join(expanded_path, "gpt_neox.embed_in.weight")
        if not os.path.exists(check_path):
            download_neox_weights(self.config.name, self.path)

        self.layers[j].init_weight(self.weight_home[j], expanded_path)

    def load_weight(self, i, j, k, overlap=True):
        logging.debug(f"<NeoXLM.load_weight> enter.")
        # Handle corner cases
    ...
    def delete_weight(self, j, k):
        logging.debug(f"<NeoXLM.delete_weight> enter.")
        if k == 0:
    ...
    def load_cache(self, i, j, k, overlap=True):
        logging.debug(f"<NeoXLM.load_cache> enter.")
        # Handle corner cases
        if i == 0:  # prefill, no cache
            return
    ...
    def store_cache(self, i, j, k, overlap=True):
        logging.debug(f"<NeoXLM.store_cache> enter.")
        # Handle corner cases
        if k == -1:
    ...
    def delete_cache(self, j, k):
        logging.debug(f"<NeoXLM.delete_cache> enter.")
    ...
    def load_hidden(self, i, j, k):
        logging.debug(f"<NeoXLM.load_hidden> enter.")
        # Handle corner cases
        if k == self.num_gpu_batches:
            k = 0
            j += 1
    ...
    def store_hidden(self, i, j, k):
        logging.debug(f"<NeoXLM.store_hidden> enter.")
        # Handle corner cases
        if k == -1:
            k = self.num_gpu_batches - 1
            j -= 1
    ...
    def compute_layer(self, i, j, k):
        logging.debug(f"<NeoXLM.compute_layer> enter i: {i}, j: {j}, k:{k}.")
    ...
    def init_all_weights(self):
        logging.debug(f"<NeoXLM.init_all_weights> enter.")
        self.weight_home = array_1d(self.num_layers, ValueHolder)
        for j in range(self.num_layers):
            self.init_weight(j)

    def delete_all_weights(self):
        logging.debug(f"<NeoXLM.delete_all_weights> enter.")
        for j in range(self.num_layers):
            self.delete_weight(j, 0)

    def update_attention_mask(self, i, k):
        logging.debug(f"<NeoXLM.update_attention_mask> enter.")
        if i > 0:
    ...
    def generate(self,
                 ...
                 verbose: int = 0):
        logging.debug(f"<NeoXLM.generate> enter.")
        task = Task(
            inputs=inputs,
    ...
    def generation_loop_normal(self):
        logging.debug(f"<NeoXLM.generation_loop_normal> enter.")
    ...
    def generation_loop_debug_normal(self):
        logging.debug(f"<NeoXLM.generation_loop_debug_normal> enter.")
    ...
    def generation_loop_overlap_single_batch(self):
        logging.debug(f"<NeoXLM.generation_loop_overlap_single_batch> enter.")
        # Prologue
        for k in range(self.num_gpu_batches):
    ...
    def generation_loop_overlap_multi_batch(self):
        logging.debug(f"<NeoXLM.generation_loop_overlap_multi_batch> enter.")
        # Prologue
        for k in range(self.num_gpu_batches):
            self.load_weight(0, 0, k)
    ...
    def generation_loop_debug_single_batch(self):
        logging.debug(f"<NeoXLM.generation_loop_debug_single_batch> enter.")
        execute_num_batches = 20
        batch_ct = 0
        pbar = tqdm(total=execute_num_batches)
    ...
    def generation_loop_debug_multi_batch(self):
        logging.debug(f"<NeoXLM.generation_loop_debug_multi_batch> enter.")
        execute_num_batches = 20
        batch_ct = 0
        pbar = tqdm(total=execute_num_batches)
        timers("prefill").reset()
...
def run_flexgen(args):
    ...
    neox_config = get_neox_config(args.model)
    cache_size = neox_config.cache_bytes(num_prompts, prompt_len + gen_len)
    hidden_size = neox_config.hidden_bytes(num_prompts, prompt_len + gen_len)
    print(f"model size: {neox_config.model_bytes()/GB:.3f} GB, "
          f"cache size: {cache_size/GB:.3f} GB, "
          f"hidden size (prefill): {hidden_size/GB:.3f} GB")

    print("init weight...")
    model = NeoXLM(neox_config, env, args.path, policy)
    ...
    log_str = write_benchmark_log(filename,
        neox_config.model_bytes(), cache_size, hidden_size,
...
def add_parser_arguments(parser):
    parser.add_argument("--model", type=str, default="togethercomputer/GPT-NeoXT-Chat-Base-20B",
        help="The model name.")
    parser.add_argument("--path", type=str, default="/root/flexgen_weights",
        help="The path to the model weights. If there are no cached weights, "
             "FlexGen will automatically download them from HuggingFace.")
    parser.add_argument("--offload-dir", type=str, default="/root/flexgen_offload_dir",
